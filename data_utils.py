...
import re
import argparse
AllRel = ['OTHER', 'COMPONENT-WHOLE(E2,E1)', 'INSTRUMENT-AGENCY(E2,E1)', 'MEMBER-COLLECTION(E1,E2)', 'CAUSE-EFFECT(E2,E1)', 
...
#line = '1	"A magnetic <e1>monopole</e1> is a hypothetical elementary <e2>particle</e2>."'
	...
	with open(filename, 'r') as rf:
		for line in rf:
			try:
				...
				sent = re.sub('\s+',' ',sent) ##\s(代指匹配任何空白字符，包括空格、制表符、换页符等等)
				...
				#if rel not in AllRel:
				#	AllRel.append(rel)
				#data['relation'].append(AllRel.index(rel))
